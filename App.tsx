...
const App: React.FC = () => {
  ...
  const handleLogin = (user: User) => {
    ...
    showToast(`Session established: ${user.name}`, 'success');
  };
  ...
  const NavItem = ({ view, label, icon: Icon }: { view: AppView, label: string, icon: any }) => (
    ...
    >
      <Icon size={18} className={`${currentView === view ? 'text-[#6D5DF6]' : 'text-slate-500 group-hover:text-slate-300'}`} />
      ...
      >
        <div className="h-24 flex items-center px-8 flex-shrink-0">
          <div className="flex items-center gap-3">
             <div className="w-9 h-9 bg-gradient-to-br from-[#6D5DF6] to-[#8B5CF6] rounded-xl flex items-center justify-center shadow-lg shadow-indigo-500/20">
                <Terminal size={20} className="text-white" />
             </div>
             <div>
                <span className="block text-sm font-black text-white tracking-tighter uppercase leading-none">Power OS</span>
                <span className="block text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-0.5">Inventory v4</span>
             </div>
          </div>
        </div>
        ...
        <div className="p-6 border-t border-white/5 mt-auto">
          <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/5 border border-white/5 mb-4">
            <div className="w-8 h-8 rounded-lg bg-[#6D5DF6]/20 flex items-center justify-center">
               <UserIcon size={16} className="text-[#6D5DF6]" />
            </div>
            <div className="min-w-0">
               <p className="text-[11px] font-bold text-white truncate">{currentUser.name}</p>
               <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest leading-none">{currentUser.role}</p>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-20 flex items-center justify-between px-8 border-b border-white/5 bg-[#0B0F14]/50 backdrop-blur-md z-40">
            <div className="flex items-center gap-6">
                <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden p-2 text-slate-400 hover:text-white bg-white/5 rounded-xl border border-white/5">
                  <Menu size={20} />
                </button>
                <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="hidden md:flex p-2 text-slate-400 hover:text-white bg-white/5 rounded-xl border border-white/5">
            ...
            <div className="flex items-center gap-4">
                ...
                `}>
                    ...
                    {isCloudConnected ? 'Core Online' : 'Core Desync'}
                </div>
                <button onClick={() => loadData()} className="p-2.5 text-slate-400 hover:text-[#6D5DF6] bg-white/5 border border-white/5 rounded-xl transition-all">
                  <RefreshCw size={18} />
                </button>
        ...
        <main className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <div className="max-w-7xl mx-auto h-full view-transition">
                ...
                {currentView === AppView.TRANSACTIONS && <TransactionManager inventory={items} transactions={transactions} onProcessTransaction={(tx) => { setTransactions([tx, ...transactions]); showToast('Transaction Validated', 'success'); }} onUpdateTransaction={(tx) => setTransactions(transactions.map(t => t.id === tx.id ? tx : t))} onDeleteTransaction={(id) => setTransactions(transactions.filter(t => t.id !== id))} userRole={currentUser.role} columns={tablePrefs.transactions} onToggleColumn={(id) => toggleColumn('transactions', id)} />}
