...
import { Search, Plus, Edit2, Trash2, X, AlertTriangle, Layers, MapPin, Power, PowerOff, Filter, MoreVertical, LayoutGrid, List } from 'lucide-react';
...
const InventoryTable: React.FC<InventoryTableProps> = ({ 
  ...
  const handleOpenModal = (item?: InventoryItem) => {
    if (item) {
      ...
      setEditingItem(null);
      setFormData({ status: 'active', baseUnit: 'Pcs', category: 'General', quantity: 0, unitPrice: 0, minLevel: 5 });
      setAltUnits([]);
    }
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 flex-shrink-0">
        <div className="flex items-center gap-3 bg-white/5 px-5 py-3 rounded-2xl border border-white/10 w-full md:w-96 focus-within:border-[#6D5DF6]/50 transition-all">
          <Search size={18} className="text-slate-500" />
          <input 
            type="text" 
            placeholder="Query node database..." 
            className="bg-transparent border-none focus:ring-0 text-sm w-full text-slate-200 placeholder:text-slate-600 font-medium"
            value={searchTerm}
      ...
      <div className="glass-panel rounded-[2rem] flex-1 overflow-hidden flex flex-col border border-white/5">
        <div className="overflow-auto flex-1 custom-scrollbar">
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-10 bg-[#0F172A]/90 backdrop-blur-md">
              <tr className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] border-b border-white/5">
                ...
                {isVisible('quantity') && <th className="px-10 py-6">Current Quota</th>}
                  ...
                  <tr key={item.id} className={`group hover:bg-white/[0.02] transition-all duration-300 ${isInactive ? 'opacity-30 grayscale' : ''}`}>
                    {isVisible('name') && (
                      <td className="px-10 py-7">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-slate-100 flex items-center gap-2.5">
                            {item.name} {isAlert && <div className="w-2 h-2 rounded-full bg-[#F97316] shadow-lg shadow-orange-500/40 animate-pulse" />}
                          </span>
                          <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1.5 leading-none">{item.sku}</span>
                        </div>
                      </td>
                    )}
                    {isVisible('category') && <td className="px-10 py-7"><span className="px-3 py-1 bg-white/5 border border-white/5 rounded-lg text-[10px] font-black uppercase tracking-widest text-slate-400">{item.category}</span></td>}
                    {isVisible('quantity') && (
                      <td className="px-10 py-7">
                        <div className="flex items-center gap-2.5">
                          <span className={`text-sm font-black ${isAlert ? 'text-[#F97316]' : 'text-[#22D3EE]'}`}>{item.quantity}</span>
                          <span className="text-[9px] text-slate-600 font-black uppercase tracking-widest mt-0.5">{item.baseUnit}</span>
                        </div>
                      </td>
                    )}
                    {isVisible('price') && <td className="px-10 py-7 text-right text-sm font-bold text-slate-300 tracking-tight">${item.unitPrice?.toLocaleString()}</td>}
                    <td className="px-10 py-7 text-right">
                      <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:-translate-x-2">
                        ...
                        <button onClick={() => handleOpenModal(item)} className="p-2 text-slate-500 hover:text-[#6D5DF6] hover:bg-[#6D5DF6]/10 rounded-xl transition-all"><Edit2 size={14} /></button>
          ...
          {filteredItems.length === 0 && (
            <div className="py-24 flex flex-col items-center justify-center text-slate-600 opacity-20 italic font-black uppercase tracking-[0.4em]">Zero Assets Discovered</div>
          )}
        </div>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0B0F14]/90 backdrop-blur-xl p-4 animate-in fade-in duration-300">
          <div className="glass-panel rounded-[2.5rem] w-full max-w-2xl flex flex-col max-h-[90vh] overflow-hidden border border-white/10 shadow-[0_0_100px_rgba(0,0,0,0.8)]">
            <div className="px-12 py-10 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
              <div>
                ...
                <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] mt-2 leading-none">Global Supply Chain Ledger</p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-3 hover:bg-white/5 rounded-full text-slate-500 hover:rotate-90 transition-all duration-500"><X size={28} /></button>
            </div>
            
            <div className="p-12 overflow-y-auto custom-scrollbar space-y-10">
              <div className="grid grid-cols-2 gap-8">
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Serial SKU</label>
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Asset Alias</label>
                  <input className="w-full bg-white/5 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white focus:ring-2 focus:ring-[#6D5DF6]/30 outline-none transition-all" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Quantum Server" />
                </div>
              </div>
              ...
              <div className="p-10 bg-white/[0.03] border border-white/5 rounded-[2.5rem] space-y-8">
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Base Metric</label>
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Initial Quota</label>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Valuation (USD)</label>
                  <input type="number" className="w-full bg-white/5 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none focus:ring-2 focus:ring-[#6D5DF6]/30" value={formData.unitPrice} onChange={e => setFormData({...formData, unitPrice: Number(e.target.value)})} placeholder="0.00" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1">Critical Limit</label>
                  <input type="number" className="w-full bg-white/5 border border-white/5 rounded-2xl px-6 py-4 text-sm text-white outline-none focus:ring-2 focus:ring-[#6D5DF6]/30" value={formData.minLevel} onChange={e => setFormData({...formData, minLevel: Number(e.target.value)})} placeholder="5" />
                </div>
              </div>

              <div className="pt-10 border-t border-white/5 flex justify-end gap-6">
                <button onClick={() => setIsModalOpen(false)} className="px-10 py-4 text-slate-500 font-black text-[11px] uppercase tracking-widest hover:text-white transition-all">Discard</button>
