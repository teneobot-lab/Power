...
const LoginPage: React.FC<LoginPageProps> = ({ users, onLogin, isLoadingData, settings, onUpdateSettings }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
        if (!cleanUsername || !cleanPassword) throw new Error('Parameters required');
        ...
        if (foundUser) {
            ...
            else throw new Error('Access key invalid');
        } else throw new Error('Entity unrecognized');
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F14] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute top-[-15%] left-[-10%] w-[60%] h-[60%] bg-[#6D5DF6]/5 rounded-full blur-[160px] animate-pulse" />
        <div className="absolute bottom-[-15%] right-[-10%] w-[60%] h-[60%] bg-[#22D3EE]/5 rounded-full blur-[160px] animate-pulse" style={{ animationDelay: '2s' }} />
      ...
      <div className="w-full max-w-md bg-[#0F172A]/40 backdrop-blur-3xl border border-white/5 rounded-[3rem] shadow-[0_40px_100px_rgba(0,0,0,0.6)] p-12 relative z-10 animate-in fade-in zoom-in-95 duration-700">
        <button 
            ...
            className="absolute top-10 right-10 p-2.5 text-[#6B7280] hover:text-white hover:bg-white/5 rounded-xl transition-all"
        >
            <Settings size={20} />
        </button>
        ...
        <div className="text-center mb-14">
            <div className="w-20 h-20 bg-gradient-to-tr from-[#6D5DF6] to-[#8B5CF6] rounded-[2rem] mx-auto flex items-center justify-center shadow-[0_0_40px_rgba(109,93,246,0.3)] mb-8 transform rotate-6 hover:rotate-0 transition-all duration-500">
                <Terminal size={32} className="text-white glow-purple" />
            </div>
            <h1 className="text-3xl font-black text-white mb-2 tracking-tighter uppercase font-heading">Steel Core</h1>
            <p className="text-[#6B7280] text-[10px] font-black uppercase tracking-[0.4em] leading-none">Global Ledger Interface</p>
        </div>

        <form onSubmit={handleLogin} className="space-y-8">
            <div className="group">
                <label className="block text-[10px] font-black text-[#6B7280] uppercase tracking-widest mb-3 ml-2 transition-colors group-focus-within:text-[#6D5DF6]">Identity Node</label>
                <input 
                    type="text" 
                    value={username}
                    onChange={(e) => setUsername(e.target.value)}
                    className="w-full bg-white/5 border border-white/5 rounded-2xl px-6 py-4.5 text-white outline-none focus:ring-2 focus:ring-[#6D5DF6]/50 transition-all placeholder-[#6B7280] text-sm font-bold"
                    placeholder="Identifier"
                />
            </div>

            <div className="group">
                <label className="block text-[10px] font-black text-[#6B7280] uppercase tracking-widest mb-3 ml-2 transition-colors group-focus-within:text-[#6D5DF6]">Secure Cipher</label>
                <div className="relative">
                    ...
                    <button 
                        ...
                        className="absolute right-6 top-1/2 -translate-y-1/2 text-[#6B7280] hover:text-white transition-colors"
                    >
            ...
            <button 
                ...
                className="w-full bg-[#6D5DF6] hover:bg-[#5B4EDB] text-white font-black py-5 rounded-[1.5rem] shadow-xl shadow-indigo-500/20 active:scale-95 disabled:opacity-50 transition-all flex items-center justify-center gap-4 text-[12px] uppercase tracking-[0.2em]"
            >
        ...
        <div className="mt-16 pt-8 border-t border-white/5 text-center">
             <div className="text-[10px] text-[#6B7280] uppercase tracking-[0.3em] font-black flex items-center justify-center gap-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B0F14]/95 backdrop-blur-2xl p-6">
            <div className="bg-[#0F172A] border border-white/5 rounded-[2.5rem] w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95">
                <div className="px-10 py-8 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                    ...
                    <button onClick={() => setIsConfigOpen(false)} className="p-2.5 hover:bg-white/5 rounded-xl"><X size={20} className="text-[#6B7280]" /></button>
                </div>
                <div className="p-10 space-y-8">
                    <div>
                        <label className="block text-[10px] font-black text-[#6B7280] uppercase tracking-widest mb-3 ml-1">Relay Endpoint</label>
