...
const LoginPage: React.FC<LoginPageProps> = ({ users, onLogin, isLoadingData }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
        ...
        // Cari user berdasarkan username (using name as proxy for username)
        ...
        if (foundUser) {
            if (foundUser.status === 'inactive') {
                ...
                // Simulasi password check (karena DB belum ada password, kita bypass dulu atau set default)
                // Untuk keamanan di masa depan, tambahkan kolom password di DB.
                onLogin(foundUser);
            }
        } else {
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl p-8 relative z-10 animate-in fade-in zoom-in duration-500">
        <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">Masuk untuk mengelola gudang</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
                ...
                <p className="text-[10px] text-slate-500 mt-2 text-right italic">Default login: masukkan username yang terdaftar</p>
            </div>
            ...
            <button 
                ...
                className="w-full bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-emerald-900/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                {isAnimating ? (
        ...
        <div className="mt-8 pt-6 border-t border-white/5 text-center">
            <p className="text-slate-500 text-xs">
                {isLoadingData ? 'Sedang memuat data user...' : 'SmartStock System v1.0'}
            </p>
        </div>
      </div>
    </div>
